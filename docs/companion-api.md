...
| GET | `/api/v1/teams/{department_id}/export.xlsx?period=` | РОП-выгрузка в Excel — **менеджер · дата и время · оценка** одной строкой на каждый оценённый элемент периода, плюс лист «Сводка» (средний балл по менеджеру). `period` принимает всё, что понимает `parse_period` — месяц `YYYY-MM`, **неделю** `YYYY-MM-DD..YYYY-MM-DD`, день. Отдел решает содержимое: ТМ → лист «Звонки» (тип звонка, ОКК 1–5, зона, «учитывается в ОКК», ссылка на CRM-карточку), встречный офис → лист «Встречи» (запись, сдано/не сдано, на проверке). Проценты берутся **с учётом апелляций**, поэтому цифра в файле равна цифре в кабинете. Отдаёт `.xlsx` (`Content-Disposition: attachment`), а не JSON (**head only**; РОП офиса — только свой отдел) |
