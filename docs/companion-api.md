...
| GET | `/api/v1/teams/{department_id}/export.xlsx?period=` | РОП-выгрузка в Excel — **менеджер · дата и время · оценка** одной строкой на каждый оценённый элемент периода, плюс лист «Сводка» (средний балл по менеджеру). `period` принимает всё, что понимает `parse_period` — месяц `YYYY-MM`, **неделю или любой диапазон дней включительно** `YYYY-MM-DD..YYYY-MM-DD` (кабинет шлёт сюда и «Период…» из карточки выгрузки), день. Длина диапазона не ограничена — в отличие от `/task-flow/export.xlsx` это чтение из своей БД, а не живой Bitrix. Отдел решает содержимое: ТМ → лист «Звонки» (тип звонка, ОКК 1–5, зона, «учитывается в ОКК», ссылка на CRM-карточку), встречный офис → лист «Встречи» (запись, сдано/не сдано, на проверке). Проценты берутся **с учётом апелляций**, поэтому цифра в файле равна цифре в кабинете. Отдаёт `.xlsx` (`Content-Disposition: attachment`), а не JSON (**head only**; РОП офиса — только свой отдел) |
...
| GET | `/api/v1/teams/{department_id}/task-flow?date=YYYY-MM-DD` | РОП «Задачи за день» — одна строка на менеджера отдела: **остаток дел на 10:00 / 14:00 / 18:00** (часы настраиваются, `companion_task_checkpoints`), **новых задач** за день, **время на линии** (разговорные секунды по отвеченным звонкам) и **встреч назначено** (`meetings_set` — сделки, вошедшие за день в стадию записи на встречу `companion_meeting_set_stage_id`, по `ASSIGNED_BY_ID`; та же цифра и тот же кэш, что у «назначено сегодня» в «Важных цифрах дня» и у «назначено» в аналитике. Вне ТМ-отдела (`companion_tm_department_id`) воронка не применяется и поле приходит `null`, не `0`; `null` и когда стейдж-история не прочиталась — остальные колонки при этом остаются). Срезы **реконструируются**, а не снимаются: задача была открыта в `T`, если создана до `T` и либо ещё открыта, либо закрыта в `T` или позже (закрытие штампует `LAST_UPDATED` — сверено с `closedDate` самой задачи). Популяция — рабочий день: дела с дедлайном этого дня и просроченные с прошлых; автозвонки телефонии (`VOXIMPLANT_CALL`/`CRM_CALL`, ~85% всех активностей) исключены. Срез, до которого день ещё не дошёл, приходит `open_tasks: null` (не ноль). Сортировка — по остатку на последнем состоявшемся срезе (кто хуже разгрёб — сверху). Три сканирования + один телефонный запрос на **весь отдел**, TTL-кэш `companion_task_flow_cache_ttl_seconds`; при упоре в `companion_task_flow_max_scan` — флаг `truncated`, при недоступном Bitrix — `data_ready: false` (**head only**; РОП офиса — только свой отдел) |
| GET | `/api/v1/teams/{department_id}/task-flow/export.xlsx?period=` | Excel-выгрузка «Задач за день» за **день** (`YYYY-MM-DD`), **произвольный диапазон дней включительно** (`YYYY-MM-DD..YYYY-MM-DD` — на нём же сидит «неделя» пн..вс) или **месяц** (`YYYY-MM`; по умолчанию текущий): лист «Задачи по дням» — строка на менеджера на каждый день периода (дата · менеджер · остаток на срезах · новых · на линии · встреч назн.), плюс лист «Сводка» — средний остаток по каждому срезу за период, суммы новых/времени/встреч на менеджера. **Диапазон читается одним набором сканов на весь период** (`day.team_task_flow_range`), а не по дню за раз — иначе месяц стоил бы ~30× round-trip'ов; кап скана свой, повышенный (`companion_task_flow_export_max_scan`). Всё равно это живой Bitrix: холодный месяц идёт минутами (у прокси кабинета для этого пути отдельный `proxy_read_timeout 600s`). Период длиннее `companion_task_flow_export_max_days` → `422`. День, до первого среза которого ещё не дошло время, в выгрузку не попадает (строки нулей не выдумываем); при недоступном Bitrix лист «Сводка» несёт строку «⚠ Bitrix не ответил» (**head only**; РОП офиса — только свой отдел) |
...
web/api/v1/
  ...
  export.py    # РОП .xlsx export (rows + rollup → openpyxl workbook)
