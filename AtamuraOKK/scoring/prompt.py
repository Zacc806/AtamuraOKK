...
applicable=true) и коротко — по ОДНОМУ короткому предложению (до 15 слов): \
justification (почему не выполнен, рус.), evidence (короткая цитата из разговора \
на языке оригинала до 15 слов или пусто) и recommendation (что сделать иначе на \
следующем звонке, рус.). Для элементов со score=1 и для Н.П. (applicable=false) \
НЕ возвращай эти три поля вовсе — обоснование выполненного элемента и так очевидно \
из чек-листа, а Н.П. в отчёт не попадает. Это экономит вывод; на итоговый балл \
текст не влияет.
...
def build_prompt(
    ...
    task = (
        ...
        "элементов со score=0 и applicable=true (по одному короткому предложению), "
        "у остальных эти поля не возвращай вовсе."
    )
