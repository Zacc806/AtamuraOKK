...
- `ingestion/` — `service.run_ingestion()` pulls `voximplant.statistic.get` since a stored cursor (`IngestState`), upserts on `bitrix_call_id` (never clobbering pipeline-owned columns), attributes each call to a `Manager`, and computes **analysis scope**. Scope = every call until the client's qualification moment (`client_qualified_at`, earliest qualified-stage entry in deal stage-history, `qualification.py`); calls after it are skipped as `after_qualification`, never-/unknown-qualified clients stay fully in scope. The pre-2026-06-12 rule's skips (`not_first_call`/`not_qualified`/`qualification_unknown`) are frozen forward-only. `refresh_qualification()` periodically re-checks recent unclaimed NEW calls so a late-arriving qualification skips post-qualification calls before they score. `download.py` is a separate stage that fetches recordings to object storage.
...
- `scoring/` — runs the active rubric (`rubric.py` / seeded `RubricVersion`) through an LLM that returns the **structured** `CallScore` (`base.py`) via forced tool-use; the worker derives numeric total/percent/zone. Only genuine `квалификация` (qualification) calls count toward the team score; reminders/vendor/internal calls are classified out. **Automatic scoring is today-only**: with `score_auto_today_only` (default True), the dispatcher's score stage and the legacy worker only claim `TRANSCRIBED` calls whose `started_at` is today (report tz) — older calls park in `TRANSCRIBED` and are scored on demand via `python -m AtamuraOKK.scoring run --all`. The date window lives in `claim_ready(..., since=)` (the single chokepoint both the dispatcher and `score_pending` share); set `ATAMURAOKK_SCORE_AUTO_TODAY_ONLY=false` to auto-score the whole backlog.
...
**Web app**: `AtamuraOKK/web/` is a FastAPI app (from `fastapi_template`) with health/monitoring + echo endpoints, Swagger at `/api/docs`, and the companion read API under `/api/v1` (see `docs/companion-api.md`). The companion API has two auth layers: the shared service bearer (nginx-injected by the sales-companion BFF) plus a personal `X-Companion-User-Key` — either the **static РОП key** (`ATAMURAOKK_COMPANION_HEAD_KEY`, grants the *global* `head` with no DB row) or a `companion_users` row whose role scopes access — `manager` sees only their own data, `head` (РОП) sees everything, and a head row with a `department_id` (Bitrix dept id) is an **office РОП** scoped to that one department (their managers + their team summary only; `ensure_can_view_manager` resolves the target manager's department from `managers`→`departments`, so unenriched managers stay global-head-only). Access management (`/api/v1/users`, the API's only writable surface) is head-tiered: any head issues/revokes **manager** keys — a scoped head only within their department, and a key they issue *ties* the manager to that department (`managers` row get-or-created, pointed at the head's department, marked `enriched` so ingestion never re-derives it); the *global* head additionally mints/revokes department-scoped head keys (`{role: "head", department_id, …}`). The cabinet can never mint or revoke a *global* head — that stays env (`ATAMURAOKK_COMPANION_HEAD_KEY`) / CLI (`python -m AtamuraOKK.companion_users create --role head [--department-id N]`, also the only reactivation path). The only non-JSON responses are the two РОП Excel exports (`web/api/v1/export.py` via openpyxl, both head-scoped like the team summary): **scores** (`GET /teams/{id}/export.xlsx?period=`) — a row per scored item of the department, менеджер · дата+время · оценка, for a month, a week **or any custom day range** (`parse_period` handles all three; the cabinet's «Период» picker sends `YYYY-MM-DD..YYYY-MM-DD`), plus a per-manager rollup sheet, appeal-corrected like the rest of the read layer; and **«Задачи за день»** (`GET /teams/{id}/task-flow/export.xlsx?period=`) — a row per manager per day of a day/week/month/custom range, mirroring the Команда card's table, plus a rollup. The latter is a *live Bitrix* read: it goes through `day.team_task_flow_range`, which reads the whole period in **one** set of scans and buckets per day (running the day view N times would cost N× the round-trips), is capped by `companion_task_flow_export_max_days`, and still takes minutes for a cold month. It is *not* the pipeline driver — the pipeline runs via the module CLIs and `worker.py`.
